...
| `python-baked-deps-ci.yml` | Digest-validated baked dependency image, frozen inexact uv sync, Python quality, audit, and build |
...
Python services whose approved runtime dependency is supplied by a Tesserix base image use
`python-baked-deps-ci.yml`. The caller must pin the image by full SHA-256 digest. A non-container
preflight validates that reference before the quality job is eligible to start the image.
...
Repository-specific coverage baselines, pruned ESLint suppressions, expiring
`audit-ci` advisory exceptions, schema bootstrap,
